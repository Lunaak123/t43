...
function highlightSelection() {
    ...
    rows.forEach((row, rowIndex) => {
        if (rowIndex >= rowFrom && rowIndex <= rowTo) {
            row.style.backgroundColor = '#fff9c4'; // Highlight entire row

            operationColumns.forEach(col => {
                const colIndex = col.charCodeAt(0) - 65; // Convert column letter to index
                if (row.cells[colIndex]) {
                    row.cells[colIndex].style.backgroundColor = '#fff9c4'; // Highlight specified cells in row
                }
            });
        }
    });
...
document.getElementById('primary-column').addEventListener('input', highlightSelection);
